<template>
  <svg class="pure-vue-bar-chart" :width="fullSvgWidth" :height="fullSvgHeight" aria-labelledby="title" role="img">
    ...
    <g :transform="`translate(0,${showYAxis ? extraTopHeightForYAxisLabel : 0})`">
      <g :transform="`translate(${showYAxis ? yAxisWidth : 0},0)`" :width="innerChartWidth" :height="innerChartHeight">
        ...
        >
          <title>
            <slot name="title" :bar-index="index" :static-value="staticValue" > <tspan>{{ staticValue }}</tspan> </slot>
          </title>
          <rect :width="width" :height="height" :x="2" :y="yOffset" :style="{ fill: barColor }" />
          <text
            v-if="showValues"
            :x="midPoint"
            :y="yOffset"
          ...
          <g v-if="showXAxis">
            <slot name="label" :bar-index="index" :mid-point="midPoint" :y-label-offset="yLabelOffset">
              <text :x="midPoint" :y="`${yLabelOffset + 10}px`" text-anchor="middle">
                {{ label }}
              </text>
            </slot>
            <line
...
export default {
  props: {
    ...
    animationDuration: { type: Number, default: 4 },
  },
  data() {
    return {
  ...
  computed: {
    ...
    dataPoints() {
      return this.usingObjectsForDataPoints ? this.points.map((item) => item.value) : this.points;
    },
    dataLabels() {
    ...
    trendLine() {
      ...
      return {
        ...
        y1: this.y(slopeValues[0]) ? this.roundTo(this.innerChartHeight - this.y(slopeValues[0]), 2) : 0,
